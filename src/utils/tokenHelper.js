 ...
 * Single-use — JTI is stored in Redis and deleted on first use.
 * Uses a dedicated secret so it can never be replayed as a reset token.
 *
 * @param {object} payload - { userId }
 * @param {string} secret - JWT signing secret (JWT_EMAIL_VERIFY_SECRET)
 * @param {number} expiresIn - Expiry in seconds, e.g., 86400 for 24 h
 * @returns {{ token: string, jti: string }} Signed token + its unique ID
 */
export function signEmailVerifyToken(payload, secret, expiresIn) {
    const jti = uuidv4();
    const token = jwt.sign(
        { userId: payload.userId },
        secret,
 ...
 * Single-use — JTI is stored in Redis and deleted on first use.
 * Uses a dedicated secret so a reset token can never be used as a verify token.
 *
 * @param {object} payload - { userId }
 * @param {string} secret - JWT signing secret (JWT_PASSWORD_RESET_SECRET)
 * @param {number} expiresIn - Expiry in seconds, e.g., 3600 for 1 h
 * @returns {{ token: string, jti: string }} Signed token + its unique ID
 */
export function signPasswordResetToken(payload, secret, expiresIn) {
    const jti = uuidv4();
    const token = jwt.sign(
        { userId: payload.userId },
        secret,
